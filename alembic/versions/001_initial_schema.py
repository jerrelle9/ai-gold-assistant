...
def upgrade() -> None:
    ...
    op.create_index("ix_indicators_symbol_date", "technical_indicators", ["symbol", "date"])
    ...
    op.create_table(
        ...
        sa.Column("price_at_detection", sa.Numeric(12, 5), nullable=True),
        sa.Column("notes", sa.Text(), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index("ix_patterns_symbol_ts", "detected_patterns", ["symbol", "detected_at"])

    op.create_table(
        "alerts",
        sa.Column("id", sa.Integer(), autoincrement=True, nullable=False),
        sa.Column("pattern_id", sa.Integer(), sa.ForeignKey("detected_patterns.id"), nullable=True),
        sa.Column("symbol", sa.String(20), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("gold_asian_range_pips", sa.Numeric(10, 2), nullable=True),
